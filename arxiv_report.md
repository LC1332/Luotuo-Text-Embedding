...
陈舒年 *,  冷子昂 *, 胡婧 *,  Liu Siyi *, 陈启源 *,   HS *, 641 *, 李鲁鲁 *
...
Liu Siyi 实现了训练的代码，并完成了基于GLM的text embedding训练
...
一般主流的做法，是将数据库的语料切割为chunk。然后在特定应用调用LLM的时候，给出一定数量的chunks作为context。这样往往可以提升语言模型系统对于专业领域知识的记忆能力。最常见的应用是在特定的知识问答[引用]，在一些角色扮演的chatbot中，也可以看到这样的使用[引用ChatHaruhi]。
